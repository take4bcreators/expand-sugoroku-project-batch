...
# ファイル名指定
source_csv_main="./job_definition_csv/job_definition_main.csv"
...
add_uuid_csv_main="./tmp/job_definition_main_uuid.csv"
tmp_ref_job_yaml="./tmp/tmp_ref_job.yaml"
create_yaml_main="./tmp/job_def_main.yaml"


# 開始メッセージ
echo "Rundeck登録処理開始（mainジョブ）"

# ディレクトリ移動
cd $(dirname $0)

: > "${add_uuid_csv_main}"
for record in $(tail -n +2 "${source_csv_main}"); do
    oneuuid=$(uuidgen)
    echo "${record}" | awk -v oneuuid="${oneuuid}" 'BEGIN{OFS=","}{print oneuuid, $0}' >> "${add_uuid_csv_main}"
done

: > "${create_yaml_main}"
for record in $(cat "${add_uuid_csv_main}"); do
    # 変数登録
    ...
    : > "${tmp_ref_job_yaml}"
    for ref_record in $(cat "${add_uuid_csv_group}"); do
    ...
    sed \
        ...
        -e "/<JOB_REF>/d" \
        "${template_yaml_ref_base}" >> "${create_yaml_main}"
done

# グループジョブを登録
rd jobs load -p "expand-sugoroku-project" -f "${create_yaml_main}" -F yaml
return_code=$?

# 終了メッセージ
echo "Rundeck登録処理完了（mainジョブ）"
