...
# JSONファイルコピー
logmsg ${LL_INFO} "コピー元ファイル：${BOARD_DATA_JSON}"
logmsg ${LL_INFO} "コピー先ディレクトリ：${FRONT_SAVE_JSON_DIR}"
cp -f "${BOARD_DATA_JSON}" "${FRONT_SAVE_JSON_DIR}/" > "${STD_OUT_FILE}" 2> "${STD_ERR_FILE}"
return_code=$?
